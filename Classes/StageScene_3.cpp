...
#define initialSpeed 0.009

bool StageScene_3::init()
{
	GameScene::init();
	srand(time(NULL));
...
void StageScene_3::Gameover()
{
	int high = readHigh("white");
	if (score <= high)
	{
...
void StageScene_3::createRestart()
{
	...
	MenuItemLabel* re = MenuItemLabel::create(gameoverLable, CC_CALLBACK_0(StageScene_3::restart, this));
...
const char* StageScene_3::getGroundMap(int m)
{
	m = m % 8;
	switch (m)
	{
...
const char* StageScene_3::getColorMap(int m)
{
	m = m % 3;
	switch (m)
	{
